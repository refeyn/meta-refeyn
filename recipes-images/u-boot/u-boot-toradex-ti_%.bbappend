...
# Torizon OTA stuff
# should be: require recipes-bsp/u-boot/u-boot-ota.inc
# but that file doesn't restrict which machines the dodeploy append acts on, so it tried to add version stuff to the R5 uboot binary
...
do_compile:append:aquila-am69-refeyn () {
...
do_deploy:append:aquila-am69-refeyn () {
    if [ -n "${UBOOT_CONFIG}" ]; then
        unset i j
        for config in ${UBOOT_MACHINE}; do
            i=$(expr $i + 1);
